...
export async function updateProfile(): Promise<void> {
  ...
  const selected = await window.showQuickPick(profileList, {
    placeHolder: l10n.t("Select a SAS Clinical Acceleration connection profile"),
  });
  if (selected) {
...
export async function deleteProfile(): Promise<void> {
  const profileList = profileConfig.listProfile();
  if (profileList.length === 0) {
    window.showErrorMessage(l10n.t("No SAS Clinical Acceleration Profiles available to delete"));
    return;
  }
  const selected = await window.showQuickPick(profileList, {
    placeHolder: l10n.t("Select a SAS Clinical Acceleration connection profile"),
  ...
  if (selected) {
    ...
    window.showInformationMessage(
      l10n.t(
        "The {selected} SAS Clinical Acceleration connection profile has been deleted from the settings.json file.",
        { selected },
      ),
    );
  }
