...
export class SASAuthProvider implements AuthenticationProvider, Disposable {
  static id = "SAS.ClinicalAcceleration";
  ...
  constructor(private readonly secretStorage: SecretStorage) {
    this._disposables = [
      ...
      workspace.onDidChangeConfiguration((event: ConfigurationChangeEvent) => {
        if (
          event.affectsConfiguration("SAS.ClinicalAcceleration.connectionProfiles") &&
          this._lastSession
        ) {
  ...
  async getSessions(): Promise<readonly AuthenticationSession[]> {
    ...
    if (sessions.length === 0) {
      commands.executeCommand("setContext", "SAS.ClinicalAcceleration.authorized", false);
    } else {
      ...
      commands.executeCommand("setContext", "SAS.ClinicalAcceleration.authorized", true);
    }
    return sessions;
  }
